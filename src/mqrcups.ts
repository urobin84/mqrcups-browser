...
export class Mqrcups {
  ...
  static init = (config: Config) => {
    return new Mqrcups(config);
  };

  static open = () => window.dispatchEvent(new Event('mqrcups:open'));
  static close = () => window.dispatchEvent(new Event('mqrcups:close'));
  static toggle = () => window.dispatchEvent(new Event('mqrcups:toggle'));
  ...
  setCustomerId = (customerId: string | null) => {
    ...
    window.dispatchEvent(
      new CustomEvent('mqrcups:customer:set', {
        detail: customerId,
      })
    );
  ...
  isValidCustomerId = async (customerId?: string | null) => {
    ...
    try {
      ...
      this.logger.warn('You might be on an older version of mqrcups.');
